...
- (NSArray *)titleArray{
    ...
    if (!_titleArray) {
        _titleArray = @[@"送礼选猴急，才是真兄弟",@"长短不是关键，技巧还需锻炼",@"‘操’之过急",@"女神哪里跑，我已受不了",@"九快一慢，现在正是慢的时候",@"是精子，总会发光",@"据说刷到这条的人，今日必有桃花",@"前戏是关键，老婆像初恋",@"放开人家",@"心急吃不了热豆腐",@"九快一慢，现在正是慢的时候"];
    }
    return _titleArray;
}
...
- (void)placeSubviews
{
    ...
    if (self.lastUpdatedTimeLabel.hidden) {
        ...
        if (self.hasImage == YES) {
            //wsy新加
            // 状态
            if (noConstrainsOnStatusLabel) {
                self.stateLabel.mj_x = 0;
                self.stateLabel.mj_y = 2;
                self.stateLabel.mj_w = self.mj_w;
                self.stateLabel.mj_h = 12;
            }
...
//    self.stateLabel.text = self.stateTitles[@(state)];
    ...
    if (self.stateLabel.text.length == 0) {
        self.stateLabel.text = self.titleArray[arc4random_uniform(10)];
    }
...
//    NSLog(@"%ld,@%ld",(long)state,(long)oldState);
